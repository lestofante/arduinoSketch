...
int scale;
{
  ...
  int timeout = 30000;
  while ( !bc_is_zero(exponent) && timeout )
  {
    timeout--;
...
int scale;
{
  ...
  //printf("ok3 %ld", exponent);
  /* Do the calculation. */
  while (exponent > 0)
  {
    pwrscale = 2*pwrscale;
    bc_multiply (power, power, &power, pwrscale);
    //printf("ok3.1 %ld", exponent);
    if ((exponent & 1) == 1) {
      //printf("ok3.2 %d %d", power, calcscale);
      calcscale = pwrscale + calcscale;
      bc_multiply (temp, power, &temp, calcscale);
      //printf("ok3.2 %ld", exponent);
    }
    exponent = exponent >> 1;
  }
  //printf("ok4");
